...
public class MailPollingService : BackgroundService
{
    ...
    private async Task ProcessWorkItemAsync(MailWorkItem workItem, int consumerId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Everything that happens to an individual mail lives in the pipeline; this
            // method is responsible only for deciding whether to poll at all.
            var pipeline = sp.GetRequiredService<MailProcessingPipeline>();
            ...
            if (pipeline.IsEventBudgetExhausted(job))
            {
            ...
            await FetchAndProcessEmailsAsync(
                job, mailbox, rule, credentialEncryptor, mailboxService, pipeline, inMaintenance, ct);
        }
        finally
        {
    ...
    /// <summary>
    /// Connects to the IMAP server, fetches the unseen messages from the configured folder,
    /// and hands each one to <see cref="MailProcessingPipeline"/>.
    /// </summary>
    /// <remarks>
    /// This method owns the IMAP session and nothing else: connect, authenticate, open,
    /// search, fetch, and reduce each message to an <see cref="InboundMail"/>. Every
    /// decision about what a mail <em>means</em> — claim, rule match, rate limit, event
    /// creation, disposition, notification, seen-gating — belongs to the pipeline, which is
    /// reachable from a test without an IMAP server. It previously all lived here, inline
    /// in this loop, behind a live <c>ImapClient</c>.
    /// </remarks>
    private async Task FetchAndProcessEmailsAsync(
        ...
        MailProcessingPipeline pipeline,
        bool inMaintenance,
        ...
        try
        {
            ...
            foreach (var uid in uids)
            {
                ...
                try
                {
                    ...
                    // Everything from here on is a decision about the mail, not about
                    // IMAP: claim, rule evaluation, rate limiting, event creation,
                    // disposition and notification. The pipeline owns all of it and can
                    // be exercised without a server; this loop only supplies the message
                    // and the means to flag it Seen.
                    var outcome = await pipeline.ProcessAsync(
                        job, mailbox, rule,
                        new InboundMail(claimKey, messageId, from, subject, body, message.Date.UtcDateTime, headers),
                        inMaintenance,
                        // The Seen flag is shared across every job polling this mailbox
                        // (the poll query is NotSeen), so the pipeline decides WHETHER to
                        // set it; this delegate is only the means of doing so.
                        token => folder.AddFlagsAsync(uid, MessageFlags.Seen, true, token),
                        ct);

                    if (outcome.Disposition is MailDisposition.EventCreated
                                            or MailDisposition.Deduplicated
                                            or MailDisposition.MaintenanceSuppressed)
                    {
                        matchCount++;
                    }
                }
